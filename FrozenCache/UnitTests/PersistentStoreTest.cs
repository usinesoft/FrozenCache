...
public class PersistentStoreTest
{
    ...
    public void NewStoreHasNoCollections()
    {
        ...
        CollectionAssert.IsEmpty(collections.Collections, "New store should have no collections");
    }
    ...
    public async Task FeedANonExistentCollection()
    {
        ...
        Assert.Throws<CacheException>(() => _ = store.FeedCollection("persons", "001", items),
            "should throw collection not found");
    }
    ...
    public async Task FeedTwoMillionItemsInCollection(int smallObjectSize, int largeObjectSize)
    {
        ...
        await using (var store = new DataStore(StoreName))
        {
            ...
            Benchmark(() =>
            {
                for (var i = 100; i < 1100; i++) _ = store.GetByPrimaryKey("persons", i * 100);
    ...
    public async Task FeedMultipleCollectionsAndRetrieveData()
    {
        await using (var store = new DataStore(StoreName))
        {
            ...
            var collections = store.GetCollectionInformation();
            Assert.That(collections.Collections.Count, Is.EqualTo(3),
                "Store should have three collections after feeding");

            var names = collections.Collections.Select(x=>x.Name).Order().ToList();
